...
function buildGraph(projects, opts = {}) {
  ...
  const lines = [
    ...
    '  classDef agent-active fill:#161b22,stroke:#3fb950,color:#3fb950',
    '  classDef agent-idle   fill:#161b22,stroke:#e3b341,color:#e3b341',
    '  classDef agent-done   fill:#161b22,stroke:#8b949e,color:#8b949e',
  ...
  for (let pi = 0; pi < projects.length; pi++) {
    ...
    if (team && team.members && team.members.length > 0) {
      ...
      if (lead) {
        const leadDetail   = (proj.agent_details || []).find(d => d.name === lead.name)
        const leadStatus   = leadDetail?.status || 'idle'
        const leadClass    = leadStatus === 'active' ? 'agent-active'
                           : leadStatus === 'done'   ? 'agent-done'
                           :                           'agent-idle'
        const leadModel    = (leadDetail?.model || '').replace('claude-', '').replace(/-\d{8}$/, '')
        const leadLabel    = leadModel
          ? `${lead.name}\\n${lead.agentType} · ${leadModel}`
          : `${lead.name}\\n${lead.agentType}`
        lines.push(`    ${leadId}["${leadLabel}"]:::${leadClass}`)
      }

      for (let ai = 0; ai < others.length; ai++) {
        const agent      = others[ai]
        const agentId    = `${tid}_a${ai}`
        const detail     = (proj.agent_details || []).find(d => d.name === agent.name)
        const agStatus   = detail?.status || 'idle'
        const agClass    = agStatus === 'active' ? 'agent-active'
                         : agStatus === 'done'   ? 'agent-done'
                         :                         'agent-idle'
        const modelShort = (detail?.model || '').replace('claude-', '').replace(/-\d{8}$/, '')
        const nodeLabel  = modelShort
          ? `${agent.name}\\n${agent.agentType} · ${modelShort}`
          : `${agent.name}\\n${agent.agentType}`
        lines.push(`    ${agentId}["${nodeLabel}"]:::${agClass}`)
      ...
      lines.push(`  end`)
      lines.push('')
    } else if (proj.agent_details && proj.agent_details.length > 0) {
      const tid = `T${pi}`
      lines.push(`  subgraph ${tid}_grp ["${proj.agent_details.length} subagents"]`)
      proj.agent_details.forEach((d, ai) => {
        const agClass    = d.status === 'active' ? 'agent-active'
                         : d.status === 'done'   ? 'agent-done' : 'agent-idle'
        const agentId    = `${tid}_a${ai}`
        const modelShort = (d.model || '').replace('claude-', '').replace(/-\d{8}$/, '')
        const nodeLabel  = modelShort
          ? `${d.name}\\n${d.status} · ${modelShort}`
          : `${d.name}\\n${d.status}`
        lines.push(`    ${agentId}["${nodeLabel}"]:::${agClass}`)
        lines.push(`    ${agentId} -. in .-> ${trunkId}`)
      })
      lines.push(`  end`)
      lines.push('')
    }
